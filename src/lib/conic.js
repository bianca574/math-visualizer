export function classifyConic(A, B, C, lang = 'fr') {
  const delta = B * B - 4 * A * C
  const labels = {
    fr: { parabola: 'parabole', ellipse: 'ellipse', hyperbola: 'hyperbole' },
    en: { parabola: 'parabola', ellipse: 'ellipse', hyperbola: 'hyperbola' },
  }
  const L = labels[lang]
  if (Math.abs(delta) < 1e-6) return L.parabola
  return delta < 0 ? L.ellipse : L.hyperbola
}
