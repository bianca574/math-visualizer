...
export default function Sidebar({ open, onClose }) {
    ...
    return (
        <>
            ...
            <aside
                className={`fixed z-40 inset-y-0 left-0 w-72 shrink-0 border-r border-ink-700 bg-ink-900 text-text-primary
                    ...
                    {categories.map((category) => (
                        <div key={category.id}>
                            <h3 className="px-2 mb-2 text-xs font-semibold uppercase tracking-wider text-ink-500">
                                {lang === 'en' ? category.labelEn : category.label}
                            </h3>
                        ...
                        border-l-2 ${isActive
                                                    ? 'border-amber-accent bg-ink-800 text-text-strong'
                                                    : 'border-transparent text-ink-500 hover:text-text-strong hover:bg-ink-800/60'
                                                }`
                                            }
                                        >
                                            <span>{lang === 'en' ? topic.labelEn : topic.label}</span>
